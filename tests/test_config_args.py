...
class TestArgs(dargs.ConfigFileArgs):
    a: int = dargs.arg_field(doc="A test argument", required=True, default="")

    b: int = dargs.arg_field(doc="A test argument", required=True, default="")

    c: int = dargs.arg_field(doc="A test argument", default=-1)
...
def test_normal_args():
    ...
    dargs.add_arguments(parser, TestArgs)
